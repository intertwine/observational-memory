...
from .oidc_discovery import validate_inference_base_url as _validate_xai_base_url
from .store import (
    auth_store_lock,
    load_auth_store,
    load_provider_state,
...
def _resolve_openai_chatgpt(*, force_refresh: bool, config=None) -> dict:
    ...
    client_id = str(state.get("client_id") or "").strip() or None
    # Re-validate the stored base_url on every resolve: an older / hand-edited /
    # tampered auth.json could carry a non-chatgpt.com host that would leak the
    # bearer. Pin it before we ever send the token.
    base_url = _chatgpt.validate_inference_base_url(
        str(state.get("base_url") or "").strip(),
        fallback=_chatgpt.CODEX_INFERENCE_BASE_URL,
    )
    ...
    if force_refresh or expiring:
        with auth_store_lock(config=config):
            ...
            if recheck:
                update = _chatgpt.refresh_tokens(refresh_token, client_id=client_id)
...
def _resolve_xai_oauth(*, force_refresh: bool, config=None) -> dict:
    ...
    client_id = str(state.get("client_id") or "").strip() or None
    # Re-validate the stored base_url against *.x.ai before sending the bearer
    # (defends against a tampered / stale auth.json pointing off-domain).
    base_url = _validate_xai_base_url(
        str(state.get("base_url") or "").strip(),
        fallback=_xai.XAI_INFERENCE_BASE_URL_DEFAULT,
    )
    ...
    if force_refresh or expiring:
        with auth_store_lock(config=config):
            ...
            if recheck:
                update = _xai.refresh_tokens(refresh_token, token_endpoint=token_endpoint, client_id=client_id)
                tokens["access_token"] = update["access_token"]
                tokens["refresh_token"] = update["refresh_token"]
